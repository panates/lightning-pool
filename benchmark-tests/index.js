...
async function runTest(options) {
  ...
  const printResults = function () {
    ...
    if (t > 0) console.log('Result: lightning is %', t, 'faster than generic');
    else console.log('Result: generic is %', t, 'faster than lightning');
    console.log(' ');
  };

  await promisify.fromCallback(cb =>
    runForAvg(testLoops, lightningPoolTest, cb),
  );
  await promisify.fromCallback(cb => runForAvg(testLoops, genericPoolTest, cb));
  printResults();
}
