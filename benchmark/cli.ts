...
async function main(): Promise<void> {
  const argv = await yargs(hideBin(process.argv))
    ...
    .option('scenario', {
      ...
      default: 'all',
      describe:
        `Comma-separated scenario names, "all", or "none" to run nothing - ` +
        `a trailing "*" matches by prefix, e.g. "acquire-*" (${SCENARIO_NAMES.join(', ')})`,
    })
    .option('repeats', {
      type: 'number',
      alias: 'r',
  ...
  console.log('\nDone. doc/BENCHMARKS.md has been regenerated.');
}
