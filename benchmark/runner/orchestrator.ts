...
import { generateBenchmarksMarkdown } from '../report/render-markdown.js';
...
function spawnWorker(
  ...
  return new Promise((resolve, reject) => {
    ...
    // sequentially, to avoid CPU contention skewing numbers and to get
    // clean, uncontaminated V8 JIT warm-up per run. `--expose-gc` gives
    // worker.ts a callable global.gc() so it can force a clean GC before
    // bench.run() to measure genuinely-retained heap growth.
    const child = spawn(
      process.execPath,
      [
        '--expose-gc',
...
export async function runMatrix(options: OrchestratorOptions): Promise<void> {
  // Scenarios can declare libs they don't (fully) support, mapped to a
  // short reason - those (lib, scenario) pairs are skipped entirely rather
  // than spawned and left to fail, and the report shows the disclosed
  ...
  // Regenerates doc/BENCHMARKS.md from *all* accumulated results (not
  // just the ones this invocation ran) so a filtered `--scenario`/`--lib`
  // run still leaves the doc reflecting the full picture rather than
  // narrowing it down to whatever subset was just run.
